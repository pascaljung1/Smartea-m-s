############################################################################################################################################
# $Id: 93_DbLog.pm 25394 2021-12-31 08:20:31Z DS_Starter $
#
# 93_DbLog.pm
...
sub DbLog_setVersionInfo {
  ...
  if($modules{$type}{META}{x_prereqs_src} && !$hash->{HELPER}{MODMETAABSENT}) {       # META-Daten sind vorhanden
      ...
      if($modules{$type}{META}{x_version}) {                                          # {x_version} ( nur gesetzt wenn $Id: 93_DbLog.pm 25394 2021-12-31 08:20:31Z DS_Starter $ im Kopf komplett! vorhanden )
          $modules{$type}{META}{x_version} =~ s/1\.1\.1/$v/xsg;
      } 
      else {
          $modules{$type}{META}{x_version} = $v; 
      }
      return $@ unless (FHEM::Meta::SetInternals($hash));                             # FVERSION wird gesetzt ( nur gesetzt wenn $Id: 93_DbLog.pm 25394 2021-12-31 08:20:31Z DS_Starter $ im Kopf komplett! vorhanden )
